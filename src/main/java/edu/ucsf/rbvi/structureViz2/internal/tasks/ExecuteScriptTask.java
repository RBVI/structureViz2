...
public class ExecuteScriptTask extends AbstractTask {
	...
	public void run(TaskMonitor taskMonitor) throws Exception {
		...
		String script = network.getRow(node).get(stringColumns.getSelectedValue(), String.class);
		String subScript = CytoUtils.columnSubstitution(structureManager, network, network.getRow(node), script);
		structureManager.getChimeraManager().sendChimeraCommand(subScript, false);
